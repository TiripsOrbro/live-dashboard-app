...
/** One cycle: yellow draws → red follows → hard wipe RTL → opacity fade → reset. */
...
async function handoffToDashboard(welcomeName, dest) {
    ...
    try {
        sessionStorage.setItem(
            WELCOME_PENDING_KEY,
            JSON.stringify({ welcomeName: String(welcomeName || '').trim() })
        );
    } catch {
        /* ignore */
    }
...
loginForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        await handoffToDashboard(data.welcomeName, dest);
    } catch (err) {
