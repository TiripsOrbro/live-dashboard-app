...
(function (global) {
    ...
    function tacauditAreaQuery() {
        if (marketViewActive && isMarketScope()) return '';
        return String(currentDisplayArea()?.name || '').trim();
    }

    function tacauditHrefForTile(tile) {
        if (!global.AppPaths?.tacauditAdminSummary) return '';
        const row =
            global.AppPaths.tacauditRowForAuditLabel?.(tile?.label, tile?.areaId) || '';
        return global.AppPaths.tacauditAdminSummary({
    ...
    function renderAdminAuditTile(tile, { tabbed = false, inRow = false } = {}) {
        ...
        const ariaAttr = tacauditHref
            ? ` aria-label="${escapeHtml(`${plainLabel} — open TacAudit summary`)}"`
            : '';
