const pathMatch = window.location.pathname.match(/^\/(teststore|\d{3,6})\/pest-walk(?:\/audit)?\/?$/i);
const STORE_NUMBER = pathMatch ? pathMatch[1].toLowerCase() : '';
const IS_AUDIT_VIEW = /\/pest-walk\/audit\/?$/i.test(window.location.pathname);
...
const SECTION_TAB_LABELS = {
    walkingInside: 'Inside',
    walkingOutside: 'Outside',
...
function minDurationMinutes() {
    return Number(schema?.minDurationMinutes) > 0 ? Number(schema.minDurationMinutes) : MIN_DURATION_MINUTES;
}

function minDurationMs() {
    return minDurationMinutes() * 60 * 1000;
}
...
function isMinimumDurationMet() {
    return sessionElapsedMs() >= minDurationMs();
}

function minimumDurationRemainingMs() {
    return Math.max(0, minDurationMs() - sessionElapsedMs());
}
...
function mountBackNav() {
    window.DashboardNavBack?.mountBackButton(document.getElementById('pw-nav-back'), {
...
const AUTO_COLLAPSE_SKIP_SECTIONS = new Set(['correctiveActions', 'signOff']);

function isNcAnswer(question, value) {
    return isPestYesType(question.type) && String(value || '').toLowerCase() === 'no';
}
...
async function saveSession() {
    ...
    saving = true;
    try {
        const data = await fetchJson(apiUrl('/api/pest-walk/session'), {
            method: 'PUT',
...
function renderBannerPoints(points) {
    if (!Array.isArray(points) || !points.length) return '';
    return `
        <ul class="dfsc-group-banner__points" aria-label="Areas to inspect">
            ${points.map((point) => `<li class="dfsc-group-banner__point">${escapeHtml(point)}</li>`).join('')}
        </ul>`;
}

function renderQuestion(question) {
    if (question.type === 'banner') {
        const hasPoints = Array.isArray(question.bannerPoints) && question.bannerPoints.length > 0;
        return `
            <div class="dfsc-group-banner${hasPoints ? ' dfsc-group-banner--with-points' : ''}">
                ...
                ${renderBannerPoints(question.bannerPoints)}
            </div>`;
    }
    ...
    const cardClass = [
        'dfsc-qcard',
        isPestYesType(question.type) ? 'dfsc-qcard--action-grid' : '',
    ...
    return `
        <article class="${cardClass}" data-question-id="${escapeHtml(question.id)}">
            ...
            ${renderQuestionFooter(question, { inlineButtons: isPestYesType(question.type) })}
        </article>`;
}
...
function renderCorrectiveSection() {
    const intro = (schema?.questions || []).find((q) => q.id === 'corrective_intro');
    const details = (schema?.questions || []).find((q) => q.id === 'corrective_details');
    const ncRows = collectNonCompliant();
    const ncHtml = ncRows.length
        ? ncRows
              .map(
                  (row) => `
            <div class="dfsc-action-item">
                <div class="dfsc-action-label">${escapeHtml(row.label)}</div>
                ${renderActionForm(row.questionId)}
            </div>`
              )
              .join('')
    ...
    return `
        ...
        ${details ? renderQuestion(details) : ''}
        ${ncHtml}`;
}

function renderMinDurationBanner() {
...
function updateSubmitButtonState() {
    const btn = document.getElementById('dfsc-submit-btn');
    if (!btn || btn.hidden) return;
    const ready = isMinimumDurationMet();
    btn.disabled = !ready;
    btn.title = ready ? '' : `Available after ${minDurationMinutes()} minutes on the timer`;
}

function renderSignOffSection() {
    return `
        ${renderMinDurationBanner()}
        <p class="dfsc-signoff-text">
            Name and signature of Manager completing the audit.
        </p>
        <div class="dfsc-field">
            <label for="pw-signoff-name">Manager name</label>
            <input class="dfsc-input" id="pw-signoff-name" type="text"
        ...
        <div class="dfsc-field">
            ...
            <div class="dfsc-signature-wrap">
                <canvas id="pw-signoff-signature" class="dfsc-signature-canvas" aria-label="Sign-off signature"></canvas>
                <div class="dfsc-signature-actions">
                    <button type="button" class="dfsc-btn dfsc-btn-ghost" data-clear-signature="pw-signoff-signature">Clear</button>
                </div>
            </div>
        </div>`;
}
            ...
            <div class="dfsc-tabs-sticky">
                <div class="dfsc-header-top">
                    <div class="dfsc-header-title">
                        <h1 class="dfsc-title">Weekly Pest Walk</h1>
                        <div class="dfsc-header-meta">
                            <span id="pw-timer">${escapeHtml(elapsed)}</span>
                        </div>
                    </div>
                </div>
                ${renderStepper()}
...
function renderQuestionArea({ scrollToTop = false, scrollAnchorQuestionId = null } = {}) {
    ...
    let body = '';
    if (section.id === 'correctiveActions') {
        body = `<div class="dfsc-questions">${renderCorrectiveSection()}</div>`;
    ...
    document.getElementById('dfsc-submit-btn').hidden = section.id !== 'signOff';
...
function bindSignOffSignature() {
    const pad = initSignaturePad('pw-signoff-signature');
    if (pad) {
        signaturePads.set('pw-signoff-signature', pad);
    ...
    const nameInput = document.getElementById('pw-signoff-name');
    if (nameInput) {
    ...
    app.innerHTML = `
        <div class="dfsc-shell">
            ...
            <div class="dfsc-nav-bar">
                ...
                <button type="button" class="dfsc-btn dfsc-btn-primary" id="dfsc-submit-btn" hidden>Complete Pest Walk</button>
            </div>
        </div>`;
...
function startTimer() {
    ...
    timerInterval = window.setInterval(() => {
        const el = document.getElementById('pw-timer');
        if (el) el.textContent = formatElapsed(sessionElapsedMs());
        const remainingEl = document.getElementById('pw-min-duration-remaining');
        if (remainingEl) {
            const left = minimumDurationRemainingMs();
            remainingEl.textContent = left > 0 ? formatElapsed(left) : '';
            if (left <= 0) {
                remainingEl.closest('.dfsc-time-gate')?.remove();
            }
        }
        updateSubmitButtonState();
    }, 1000);
...
async function submitAudit() {
    ...
    if (!isMinimumDurationMet()) {
        statusMessage = `Pest walk must be at least ${minDurationMinutes()} minutes. ${formatElapsed(minimumDurationRemainingMs())} remaining.`;
        statusKind = 'error';
        renderStatusBar();
        return;
    }
    const signPad = signaturePads.get('pw-signoff-signature');
    const name = document.getElementById('pw-signoff-name')?.value?.trim() || '';
    ...
    try {
        await saveSession();
        const data = await fetchJson(apiUrl('/api/pest-walk/submit'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                store: STORE_NUMBER,
    ...
    app.innerHTML = `
        <div class="dfsc-shell">
            <article class="dfsc-card">
                ...
                <button type="button" class="dfsc-btn dfsc-btn-secondary" id="pw-download-pdf">Download PDF</button>
                <button type="button" class="dfsc-btn dfsc-btn-primary" id="pw-back-dashboard">Back to TacoAudit</button>
            </article>
        </div>`;
    document.getElementById('pw-back-dashboard').addEventListener('click', () => {
        window.location.href = tacauditPath();
    });
    document.getElementById('pw-download-pdf').addEventListener('click', () => {
        window.location.href = apiUrl('/api/pest-walk/report.pdf', {
            store: STORE_NUMBER,
            sessionId: session.id,
            periodKey: session.periodKey,
        });
...
function renderOpenAuditsSection(openAudits = []) {
    ...
    const rows = openAudits
        ...
        .join('');
    return `
        <section class="dfsc-open-section" aria-labelledby="pw-open-heading">
            <div class="dfsc-open-head">
                <h2 id="pw-open-heading">Your open audits</h2>
    ...
    app.innerHTML = `
        <div class="dfsc-shell">
            ...
            <article class="dfsc-card">
                ...
                <div class="dfsc-field">
                    <label>Time</label>
                    <input class="dfsc-input dfsc-input-readonly" readonly id="pw-clock" value="${escapeHtml(context.timeLabel)}" />
                </div>
                <div class="dfsc-field">
                    <label for="pw-name">Conducted by (full name)</label>
                    <input class="dfsc-input" id="pw-name" type="text" autocomplete="name" value="${escapeHtml(context.conductorFullName || '')}" />
                </div>
                <div class="dfsc-field">
                    <span class="dfsc-field-label">Signature</span>
                    <div class="dfsc-signature-wrap">
                        <canvas id="pw-start-signature" class="dfsc-signature-canvas" aria-label="Start signature"></canvas>
                        <div class="dfsc-signature-actions">
                            <button type="button" class="dfsc-btn dfsc-btn-ghost" data-clear-signature="pw-start-signature">Clear</button>
                        </div>
                    </div>
                </div>
                <button type="button" class="dfsc-btn dfsc-btn-primary" id="pw-begin-btn">Begin Pest Walk</button>
            </article>
        </div>`;

    const startPad = initSignaturePad('pw-start-signature');
    signaturePads.set('pw-start-signature', startPad);

    document.getElementById('pw-begin-btn').addEventListener('click', () => startSession(false));
...
async function startSession(forceNew) {
    ...
    const startPad = signaturePads.get('pw-start-signature');
    ...
    try {
        const data = await fetchJson(apiUrl('/api/pest-walk/start'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                store: STORE_NUMBER,
        ...
        window.history.replaceState({}, '', `/${STORE_NUMBER}/pest-walk/audit?session=${session.id}`);
        renderAuditView();
    } catch (err) {
        statusMessage = err.message;
        statusKind = 'error';
...
async function deleteOpenAudit(sessionId) {
    ...
    try {
        await fetch(apiUrl('/api/pest-walk/session', { store: STORE_NUMBER, sessionId }), {
            method: 'DELETE',
            credentials: 'include',
        });
        context = await fetchJson(apiUrl('/api/pest-walk/context', { store: STORE_NUMBER }));
        renderLandingView();
    } catch (err) {
        statusMessage = err.message;
        statusKind = 'error';
...
async function loadSessionFromQuery() {
    ...
    try {
        const data = await fetchJson(
            apiUrl('/api/pest-walk/session', { store: STORE_NUMBER, sessionId })
        );
        session = data.session;
...
async function init() {
    ...
    try {
        const [, contextData] = await Promise.all([
            ...
            fetchJson(apiUrl('/api/pest-walk/context', { store: STORE_NUMBER })),
        ]);
        context = contextData;
        schema = context.schema;
        if (IS_AUDIT_VIEW) {
            if (await loadSessionFromQuery()) return;
            window.location.replace(`/${STORE_NUMBER}/pest-walk`);
            return;
        }
        if (await loadSessionFromQuery()) return;
        renderLandingView();
