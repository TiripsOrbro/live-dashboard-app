...
function resolveCreateAccountParent(req) {
    ...
    if (sessionUser && canUserCreateAccounts(sessionUser) && !isAdminUser(sessionUser)) {
        return {
            parentUsername: sessionUser.username,
            stores: sessionUser.stores === '*' ? [] : [...sessionUser.stores],
            addCbAlias: Boolean(sessionUser.colorBlind) || /^CB\d{3,6}$/i.test(sessionUser.username),
            via: 'session',
        };
    }
    const gate = readGateFromRequest(req);
    if (!gate) return null;
    if (gate.stores === '*' || !gate.stores.length) return null;
    return {
        parentUsername: gate.parentUsername,
        stores: [...gate.stores],
        addCbAlias: gate.addCbAlias,
        via: 'gate',
    };
}
