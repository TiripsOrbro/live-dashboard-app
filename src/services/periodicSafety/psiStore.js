...
const { getDismissalPeriodKey, getPsiRotation } = require('../../utils/auditRecurrence');
...
const { applyCollaborativeUpdates } = require('../audit/auditContributions');
const {
    buildSchemaPayload,
    validateSection,
...
const PSI_DATA_DIR = path.join(PROJECT_ROOT, 'data', 'periodic-safety');
...
function sessionDir(storeNumber, periodKey) {
    return path.join(PSI_DATA_DIR, normalizeStoreKey(storeNumber), periodKey);
}

function sessionFilePath(storeNumber, periodKey, sessionId) {
    return path.join(sessionDir(storeNumber, periodKey), `${sessionId}.json`);
}

function activePointerPath(storeNumber) {
    return path.join(PSI_DATA_DIR, normalizeStoreKey(storeNumber), '_active.json');
}
...
function pruneOldRecords(storeNumber, referencePeriodKey) {
    ...
    const storeRoot = path.join(PSI_DATA_DIR, store);
...
function getContext(storeNumber, options = {}) {
    ...
    const psiRotation = getPsiRotation();
    ...
    return {
        ...
        psiWeek: psiRotation.psiWeek,
        psiWeekTitle: psiRotation.psiWeekTitle,
        psiSlot: psiRotation.psiSlot,
        ...
        schema: buildSchemaPayload(psiRotation.psiWeek),
    };
}
...
function createSession(
    ...
    const periodKey = getDismissalPeriodKey();
    const psiRotation = getPsiRotation();
    const psiWeek = normalizePsiWeek(requestedWeek ?? psiRotation.psiWeek);
    pruneOldRecords(store, periodKey);
...
function updateSession(storeNumber, sessionId, updates = {}, access = {}) {
    ...
    if (session.status === 'completed') return { ok: false, error: 'This safety inspection is already completed.' };
    ...
    applyCollaborativeUpdates(session, updates, access, {
        getQuestionById: (questionId) => getQuestionById(questionId, session.psiWeek),
        normalizeActionUpdate,
    });
...
function submitSession(storeNumber, sessionId, signOff = {}, access = {}) {
    ...
    const currentWeek = getPsiRotation().psiWeek;
    ...
    afterAuditSubmit({ storeNumber: store, auditType: 'psi', session });
