...
const PEST_WALK_DATA_DIR = path.join(PROJECT_ROOT, 'data', 'pest-walk');
...
function sessionDir(storeNumber, periodKey) {
    return path.join(PEST_WALK_DATA_DIR, normalizeStoreKey(storeNumber), periodKey);
}
...
function activePointerPath(storeNumber) {
    return path.join(PEST_WALK_DATA_DIR, normalizeStoreKey(storeNumber), '_active.json');
}
...
function pruneOldRecords(storeNumber, referencePeriodKey) {
    ...
    const storeRoot = path.join(PEST_WALK_DATA_DIR, store);
...
function updateSession(storeNumber, sessionId, updates = {}, access = {}) {
    ...
    if (session.status === 'completed') return { ok: false, error: 'This pest walk is already completed.' };
...
function getAuditById(sessionId) {
    const storesRoot = PEST_WALK_DATA_DIR;
