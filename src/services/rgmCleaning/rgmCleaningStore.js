...
const RGM_CLEANING_DATA_DIR = path.join(PROJECT_ROOT, 'data', 'rgm-cleaning');
...
function sessionDir(storeNumber, periodKey) {
    return path.join(RGM_CLEANING_DATA_DIR, normalizeStoreKey(storeNumber), periodKey);
}
...
function loadSquareOnePhotoCandidates(storeNumber, periodKey) {
    try {
        const { listPhotoCandidatesForPeriod } = require('../squareOne/squareOneStore');
