...
const REPORT_SECTIONS = PSI_SECTIONS.filter((s) => s.id !== 'signOff');
...
function buildReportFilename(session) {
    ...
    const storeSlug = String(session.storeName || session.storeNumber || 'Store')
        ...
        .trim()
        .replace(/\s+/g, '-');
    return `${day}-${month}-${year}-PSI-Week-${session.psiWeek || '?'}-${storeSlug || 'Store'}.pdf`;
}
...
function answerClass(question, value) {
    ...
    if (isPsiYesType(question.type) && String(value).toLowerCase() === 'yes') return 'answer-ok';
    return '';
}
...
function buildPsiReportHtml(session) {
