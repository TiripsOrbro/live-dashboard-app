...
const { getDismissalPeriodKey, getPsiRotation } = require('../../src/auditRecurrence');
...
function updateSession(storeNumber, sessionId, updates = {}, access = {}) {
    ...
    syncUpdatedActionsToRegistry(store, 'psi', session, updates, access, (questionId) =>
        getQuestionById(questionId, session.psiWeek)
    );
...
function submitSession(storeNumber, sessionId, signOff = {}, access = {}) {
    ...
    promoteSessionActionsOnSubmit(store, 'psi', session, collectNonCompliant, access);
    saveSession(session);
