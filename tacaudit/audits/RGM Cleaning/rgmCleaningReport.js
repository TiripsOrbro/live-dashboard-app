...
const REPORT_SECTIONS = RGM_CLEANING_SECTIONS.filter(
    (s) => s.id !== 'signOff' && s.id !== 'squareOneReview'
);
...
function buildReportFilename(session) {
    ...
    const storeSlug = String(session.storeName || session.storeNumber || 'Store')
        ...
        .trim()
        .replace(/\s+/g, '-');
    return `${day}-${month}-${year}-RGM-Cleaning-${storeSlug || 'Store'}.pdf`;
}
...
function formatAnswerValue(question, value) {
    ...
    if (isCleaningRatingType(question.type)) {
        ...
        if (raw.toLowerCase() === 'not_satisfactory') return 'Not Satisfactory';
    }
...
function answerClass(question, value) {
    ...
    if (isCleaningRatingType(question.type) && String(value).toLowerCase() === 'satisfactory') return 'answer-ok';
    return '';
}
...
function renderQuestionRows(session, questions) {
    return questions
        ...
        .map((question) => {
            ...
            const photoBy = formatContributionLine(session, 'photos', question.id);
            return `
                <tr class="${isNc ? 'row-nc' : ''}">
                    <td class="col-question">${escapeHtml(question.label)}</td>
                    <td class="col-answer ${answerClass(question, value)}">${escapeHtml(formatAnswerValue(question, value))}${answerBy ? `<div class="contrib-meta">${escapeHtml(answerBy)}</div>` : ''}${photoBy ? `<div class="contrib-meta">Photo: ${escapeHtml(photoBy)}</div>` : ''}</td>
                </tr>
                ${
                    note
...
function buildRgmCleaningReportHtml(session) {
        ...
        const groupsHtml = groups
            .map((group) => {
                ...
                const bannerHtml = banner
                    ? `<div class="section-banner"><strong>${escapeHtml(banner.bannerTitle || '')}</strong><p>${escapeHtml(banner.bannerSubtitle || '')}</p></div>`
                    : '';
    ...
    const actionPlanHtml = actionPlanFields
        .map(([id, label]) => {
            const text = String(session.answers?.[id] || '').trim();
            if (!text) return '';
            return `<div class="action-plan-block"><strong>${escapeHtml(label)}</strong><p>${escapeHtml(text)}</p></div>`;
        })
        .filter(Boolean)
        .join('');
    const actionsHtml =
        actionPlanHtml ||
        (nc.length === 0
            ? '<p class="empty-hint">No action plan entries recorded.</p>'
...
<body>
    <header class="report-header">
        <h1 class="report-title">Weekly RGM Cleaning Assessment</h1>
    ...
    <div class="summary-strip">
        <div class="summary-pill">
            <span>Score</span>
            <strong>${score.satisfactoryCount} / ${score.total} (${score.percent}%)</strong>
        </div>
...
async function buildRgmCleaningReportPdf(session) {
    const html = buildRgmCleaningReportHtml(session);
...
module.exports = {
    buildRgmCleaningReportHtml,
    buildRgmCleaningReportPdf,
    buildReportFilename,
};
