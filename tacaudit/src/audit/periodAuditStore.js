...
const { afterAuditSubmit } = require('../core/tacauditSubmit');
const {
    userOwnsSession,
...
function createPeriodAuditStore({ auditType, dataDir, schema }) {
    const AUDIT_LABEL = schema.AUDIT_LABEL;
    ...
    function activePointerPath(storeNumber) {
        return path.join(dataDir, normalizeStoreKey(storeNumber), '_active.json');
    }
    ...
    function saveSession(session) {
        writeJson(sessionFilePath(session.storeNumber, session.periodKey, session.id), session);
        return session;
    }
    ...
    function summarizeCompletedAudit(session) {
        const nonCompliant = Array.isArray(session.nonCompliant)
            ...
            : schema.collectNonCompliant(session);
    ...
    function getContext(storeNumber, options = {}) {
        ...
        return {
            ...
            periodSummary: {
                ...
                latestScore: period.latestScore,
            },
    ...
    function getSessionById(storeNumber, sessionId, periodKey) {
        return loadSession(normalizeStoreKey(storeNumber), periodKey || getDismissalPeriodKey(), sessionId);
    }
    ...
    function updateSession(storeNumber, sessionId, updates = {}, access = {}) {
        ...
        if (session.status === 'completed') return { ok: false, error: 'This audit is already completed.' };
        ...
        return {
            ok: true,
            session,
            nonCompliant: schema.collectNonCompliant(session),
            score: schema.scoreSession(session),
        };
    }
    ...
    function reopenSession(storeNumber, sessionId, periodKey, access = {}) {
        ...
        saveSession(session);
        setActivePointer(store, session.id, session.periodKey);
        return { ok: true, session, nonCompliant: schema.collectNonCompliant(session) };
    }
    ...
    function submitSession(storeNumber, sessionId, signOff = {}, access = {}) {
        ...
        const validation = schema.validateSessionComplete(session);
    ...
    function validateSessionSection(storeNumber, sessionId, sectionId, access = {}) {
        ...
        return schema.validateSection(session, sectionId);
    }
    ...
    function listOpenAudits(storeNumber, options = {}) {
        ...
        const storeRoot = path.join(dataDir, store);
        if (!fs.existsSync(storeRoot)) return [];
        const access = options.access || options;
        const open = [];
        for (const entry of fs.readdirSync(storeRoot, { withFileTypes: true })) {
            if (!entry.isDirectory() || entry.name.startsWith('_')) continue;
    ...
    function deleteOpenAudit(storeNumber, sessionId, access = {}) {
        ...
        if (session.status === 'completed') return { ok: false, error: 'Completed audits cannot be deleted.' };
        if (!userOwnsSession(session, access.username, access.conductorFullName)) {
    ...
    function getAuditById(sessionId) {
        ...
        for (const storeEntry of fs.readdirSync(dataDir, { withFileTypes: true })) {
            if (!storeEntry.isDirectory()) continue;
            const storeRoot = path.join(dataDir, storeEntry.name);
    ...
    return {
        ...
        buildSchemaPayload: schema.buildSchemaPayload,
    };
}
