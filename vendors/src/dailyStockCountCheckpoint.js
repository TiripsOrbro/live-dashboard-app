...
const CHECKPOINT_FILE =
    process.env.DAILY_STOCK_COUNT_CHECKPOINT_FILE ||
    path.join(require('../../src/paths').vendors.data, 'daily-stock-count-pipeline.json');
...
async function readFileState() {
    try {
        ...
        if (error.code !== 'ENOENT') {
            console.warn('[DailyStockCount] Failed to read checkpoint file:', error.message);
        }
    }
    return emptyState();
}
